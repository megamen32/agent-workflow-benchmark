...
Поэтому эти пять сценариев относятся к `workflow_guard`, а не к общей
инженерной способности. Отдельный `product_outcome` трек должен проверять
сложные bugfix/feature/refactor/integration задачи скрытыми и
детерминированными продуктовыми тестами. Мы не смешиваем две оценки в один
процент.
...
но собираем все диалоги кампании в один общий JSONL перед сжатием. Один release
asset содержит только `campaign-manifest.json` и `transcripts.jsonl`; plugin
cache, homes, workdirs и отдельные архивы по run не публикуются. Это сохраняет
воспроизводимость и даёт лучший коэффициент сжатия.
